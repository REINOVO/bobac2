...
bool audio_play(string audio_file){
    ...
    if(pid < 0) {
        ...
        execlp("aplay","aplay",audio_file.c_str(),(char*)0);  //播放合成的音频
            ...
            if(!m_isTracking) {
                if  (cUsr.isNew()) {
                    ...
                    if(poseData.getType()==nite::POSE_PSI/**< 投降姿态 */ && poseData.isHeld()/**< 跟踪成功 */) {
                       ...
                       // aplay_file = "./source/AIUI/audio/start.wav";
                      //  audio_play(aplay_file);
                        m_isTracking = true;
			m_sleep = false;
                   ...
                   // cout<<"sleep_________"<< m_sleep <<endl;
                //检测到POSE_PSI时，停止骨骼跟踪
                ...
                if(poseData.getType()==nite::POSE_CROSSED_HANDS && poseData.isHeld() ) {
                   ...
                   // m_userTracker.stopSkeletonTracking(cUsr.getId());
                  //  m_isTracking = false;
                  //  aplay_file = "./source/AIUI/audio/stop.wav";
                   // audio_play(aplay_file);
                    m_sleep = true;
                ...
                if(m_userTracker.startSkeletonTracking(cUsr.getId())){
                    m_sleep = false;
                }
                   // aplay_file = "./source/AIUI/audio/start.wav";
                   // audio_play(aplay_file);
