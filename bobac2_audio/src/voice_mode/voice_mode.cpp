...
#define LOGIN_PARAMS  "appid = 5d536221, work_dir = ./voice_config/voice_order"//登录参数,appid与msc库绑定,请勿随意改动
...
bool mode_cb(bobac2_audio::Mode::Request &req, bobac2_audio::Mode::Response &res){
  std::string text;
  text = run_iat(req.voice_filename.c_str(), SESSION_BEGIN_PARAMS);//获取识别的文本
	if(text.find("聊天模式") != std::string::npos){
    res.mode= "聊天模式";
  }
	else if(text.find("控制模式") != std::string::npos){
    res.mode= "控制模式";
  }
	else if(text.find("导航模式") != std::string::npos){
    res.mode= "导航模式";
  }
  else res.mode= "未知模式";
...
int main(int argc, char* argv[])
{
  ...
  ros::ServiceServer iat_server = nh.advertiseService("voice_mode", mode_cb);
  /*登录函数*/
  Login(LOGIN_PARAMS);
  return 0;
}
